...
namespace
{
    ...
    int generic_induced_subgraph_test(const hipgraph_resource_handle_t* p_handle,
                                      vertex_t*                         h_src,
                                      vertex_t*                         h_dst,
                                      weight_t*                         h_wgt,
                                      size_t                            num_vertices,
                                      ...
                                      size_t                            num_results)
    {
        ...
        EXPECT_EQ(ret_code, HIPGRAPH_SUCCESS) << "subgraph_offsets create failed.";

        ret_code = hipgraph_type_erased_device_array_create(
            p_handle, num_subgraph_vertices, vertex_tid, &subgraph_vertices, &ret_error);
        EXPECT_EQ(ret_code, HIPGRAPH_SUCCESS) << "subgraph_offsets create failed.";
        ...
        EXPECT_EQ(ret_code, HIPGRAPH_SUCCESS) << "subgraph_offsets copy_from_host failed.";

        ret_code = hipgraph_type_erased_device_array_view_copy_from_host(
            p_handle, subgraph_vertices_view, (hipgraph_byte_t*)h_subgraph_vertices, &ret_error);
        EXPECT_EQ(ret_code, HIPGRAPH_SUCCESS) << "subgraph_vertices copy_from_host failed.";
        ...
        for(size_t i = 0; (i < extracted_size) && (test_ret_value == 0); ++i)
        {
            ...
            EXPECT_TRUE(found) << "extracted an edge that doesn't match";
        }

        hipgraph_type_erased_device_array_free(subgraph_offsets);
        hipgraph_type_erased_device_array_free(subgraph_vertices);
    ...
    int test_induced_subgraph(const hipgraph_resource_handle_t* p_handle)
    {
        size_t num_edges            = 8;
        ...
        return generic_induced_subgraph_test(p_handle,
                                             h_src,
                                             h_dst,
                                             h_wgt,
                                             num_vertices,
