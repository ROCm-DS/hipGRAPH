...
namespace
{
    ...
    int generic_triangle_count_test(const hipgraph_resource_handle_t* p_handle,
                                    vertex_t*                         h_src,
                                    vertex_t*                         h_dst,
                                    weight_t*                         h_wgt,
                                    vertex_t*                         h_verts,
                                    ...
                                    hipgraph_bool_t                   store_transposed)
    {
        ...
        if(h_verts != nullptr)
        {
            if(rank == 0)
            {
                ...
                EXPECT_EQ(ret_code, HIPGRAPH_SUCCESS) << "p_start create failed.";
        ...
        if(test_ret_value == 0)
        {
            ...
            for(int i = 0; (i < num_local_results) && (test_ret_value == 0); ++i)
            {
                for(int j = 0; j < num_results; ++j)
                {
                    ...
                    {
                        EXPECT_EQ(h_counts[i], h_result[j]) << "counts results don't match";
                    }
                }
            }
    ...
    int test_triangle_count(const hipgraph_resource_handle_t* p_handle)
    {
        size_t num_edges    = 16;
        ...
        return generic_triangle_count_test(p_handle,
                                           h_src,
                                           h_dst,
                                           h_wgt,
                                           h_verts,
    ...
    int test_triangle_count_dolphins(const hipgraph_resource_handle_t* p_handle)
    {
