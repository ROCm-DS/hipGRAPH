...
namespace
{
    ...
    void generic_wcc_test(vertex_t*       h_src,
                          vertex_t*       h_dst,
                          weight_t*       h_wgt,
                          vertex_t*       h_result,
                          size_t          num_vertices,
                          ...
                          hipgraph_bool_t store_transposed)
    {
        ...
        EXPECT_EQ(ret_code, HIPGRAPH_SUCCESS)
            << "hipgraph_weakly_connected_components failed: " << hipgraph_error_message(ret_error);
        ...
        for(size_t i = 0; i < num_vertices; ++i)
        {
            if(component_check[h_result[h_vertices[i]]] == vertex_t(num_vertices))
            {
        ...
        for(size_t i = 0; i < num_vertices; ++i)
        {
            EXPECT_EQ(h_components[i], component_check[h_result[h_vertices[i]]])
    ...
    TEST(AlgorithmTest, WeaklyConnectedComponents)
    {
        size_t num_edges    = 32;
        ...
        generic_wcc_test(h_src, h_dst, h_wgt, h_result, num_vertices, num_edges, HIPGRAPH_FALSE);
    }

    TEST(AlgorithmTest, WeaklyConnectedComponentsTranspose)
    {
        size_t num_edges    = 32;
        size_t num_vertices = 12;
        ...
        // WCC wants store_transposed = HIPGRAPH_FALSE
        generic_wcc_test(h_src, h_dst, h_wgt, h_result, num_vertices, num_edges, HIPGRAPH_TRUE);
    }
