...
namespace
{
    ...
    int test_rmat_generation(const hipgraph_resource_handle_t* p_handle)
    {
...
// assumes that the underlying random number generator produces a known sequence)
        ...
        int rank = hipgraph_resource_handle_get_rank(handle);

        ret_code = hipgraph_rng_state_create(handle, rank, &rng_state, &ret_error);
        ...
        ret_code = hipgraph_generate_rmat_edgelist(
            handle, rng_state, 5, 30, 0.57, 0.19, 0.19, HIPGRAPH_FALSE, &coo, &ret_error);
        EXPECT_EQ(ret_code, HIPGRAPH_SUCCESS) << "generate_rmat_edgelist failed.";
        ...
        if(rank < max_rank_to_validate)
        {
            ...
            ret_code = hipgraph_type_erased_device_array_view_copy_to_host(
                handle, (hipgraph_byte_t*)h_src, src_view, &ret_error);
            EXPECT_EQ(ret_code, HIPGRAPH_SUCCESS) << "src copy_to_host failed.";

            ret_code = hipgraph_type_erased_device_array_view_copy_to_host(
                handle, (hipgraph_byte_t*)h_dst, dst_view, &ret_error);
            EXPECT_EQ(ret_code, HIPGRAPH_SUCCESS) << "dst copy_to_host failed.";

            EXPECT_EQ(src_size, expected_len[rank]) << "incorrect number of edges";

            for(int i = 0; (i < src_size) && (test_ret_value == 0); ++i)
            {
                EXPECT_EQ(expected_src[rank][i], h_src[i]) << "generated edges don't match";
                EXPECT_EQ(expected_dst[rank][i], h_dst[i]) << "generated edges don't match";
            }
    ...
    int test_rmat_list_generation(const hipgraph_resource_handle_t* p_handle)
    {
// FIXME: this should be re-implemented (this test doesn't work for more than 8 GPUs, and this
// assumes that the underlying random number generator produces a known sequence)
#if 1
        ...
        vertex_t expected_src_0_rank_0[]
            = {29, 16, 16, 5, 6, 1, 20, 5, 22, 14, 3, 12, 4, 0, 10, 0, 4, 16, 20, 16};
        vertex_t expected_dst_0_rank_0[]
            = {0, 18, 0, 8, 1, 0, 11, 8, 21, 16, 0, 2, 2, 0, 17, 0, 9, 0, 8, 0};
        ...
        vertex_t expected_src_2_rank_0[]
            = {5, 0, 24, 11, 2, 20, 2, 1, 0, 18, 2, 0, 0, 0, 8, 4, 1, 22, 0, 1};
        vertex_t expected_dst_2_rank_0[]
            = {16, 4, 0, 1, 8, 0, 0, 0, 8, 7, 3, 15, 0, 0, 4, 26, 16, 0, 8, 2};
        vertex_t expected_src_0_rank_1[]
            = {0, 21, 10, 17, 8, 0, 21, 24, 0, 0, 0, 1, 2, 16, 0, 2, 2, 24, 0, 27};
        vertex_t expected_dst_0_rank_1[]
            = {0, 2, 4, 0, 19, 12, 0, 18, 4, 16, 8, 0, 1, 4, 5, 4, 18, 16, 20, 24};
        ...
        vertex_t expected_src_2_rank_1[]
            = {4, 12, 0, 0, 1, 12, 2, 0, 16, 0, 20, 8, 14, 0, 16, 9, 24, 8, 10, 1};
        vertex_t expected_dst_2_rank_1[]
            = {16, 0, 0, 4, 0, 13, 0, 1, 2, 0, 0, 8, 8, 4, 1, 3, 5, 16, 2, 0};
        ...
        vertex_t expected_src_1_rank_2[]
            = {18, 12, 2, 0, 12, 4, 1, 18, 0, 4, 0, 3, 0, 0, 18, 22, 3, 4, 2, 8};
        vertex_t expected_dst_1_rank_2[]
            = {2, 1, 8, 22, 1, 28, 4, 11, 0, 0, 12, 24, 8, 3, 8, 6, 1, 16, 0, 2};
        ...
        vertex_t expected_src_0_rank_3[]
            = {0, 13, 0, 2, 16, 4, 10, 1, 20, 2, 0, 0, 1, 0, 16, 3, 6, 10, 0, 0};
        vertex_t expected_dst_0_rank_3[]
            = {9, 16, 0, 8, 9, 1, 0, 1, 3, 0, 0, 2, 8, 0, 12, 2, 20, 8, 29, 3};
        vertex_t expected_src_1_rank_3[]
            = {2, 13, 3, 2, 18, 8, 0, 0, 8, 10, 17, 0, 2, 26, 9, 28, 1, 0, 10, 17};
        vertex_t expected_dst_1_rank_3[]
            = {2, 8, 0, 14, 16, 20, 8, 8, 0, 0, 7, 0, 16, 4, 20, 16, 5, 3, 0, 3};
        ...
        vertex_t expected_src_2_rank_4[]
            = {9, 2, 0, 20, 0, 3, 5, 22, 2, 8, 26, 0, 18, 16, 12, 0, 0, 18, 8, 2};
        vertex_t expected_dst_2_rank_4[]
            = {2, 8, 6, 18, 16, 16, 19, 0, 16, 1, 0, 0, 12, 0, 8, 0, 0, 8, 22, 4};
        ...
        vertex_t expected_src_1_rank_5[]
            = {16, 0, 8, 17, 9, 8, 2, 1, 0, 24, 8, 2, 0, 0, 16, 22, 16, 0, 0, 2};
        vertex_t expected_dst_1_rank_5[]
            = {8, 10, 2, 0, 0, 6, 8, 2, 8, 16, 1, 0, 24, 26, 0, 0, 2, 3, 8, 24};
        vertex_t expected_src_2_rank_5[]
            = {21, 5, 10, 24, 1, 16, 10, 0, 2, 2, 5, 4, 2, 0, 0, 8, 2, 0, 4, 4};
        vertex_t expected_dst_2_rank_5[]
            = {4, 14, 20, 0, 0, 8, 20, 1, 8, 21, 16, 2, 0, 2, 14, 0, 10, 24, 9, 5};
        vertex_t expected_src_0_rank_6[]
            = {8, 16, 2, 8, 0, 24, 20, 10, 8, 0, 0, 0, 20, 8, 10, 0, 4, 8, 12, 4};
        vertex_t expected_dst_0_rank_6[]
            = {21, 25, 6, 6, 2, 0, 12, 1, 8, 8, 16, 28, 8, 1, 10, 0, 1, 0, 16, 2};
        ...
        vertex_t expected_src_2_rank_6[]
            = {2, 4, 18, 18, 0, 24, 4, 16, 5, 18, 17, 13, 16, 28, 24, 0, 28, 0, 13, 1};
        vertex_t expected_dst_2_rank_6[]
            = {8, 24, 24, 1, 16, 2, 0, 4, 1, 0, 10, 10, 18, 0, 18, 18, 2, 26, 8, 0};
        vertex_t expected_src_0_rank_7[]
            = {0, 5, 0, 4, 2, 3, 4, 4, 20, 18, 0, 0, 29, 17, 0, 0, 0, 1, 0, 8};
        vertex_t expected_dst_0_rank_7[]
            = {4, 0, 24, 0, 0, 18, 8, 0, 0, 9, 3, 12, 4, 8, 17, 1, 0, 28, 22, 1};
        ...
        vertex_t expected_src_2_rank_7[]
            = {8, 9, 2, 4, 4, 0, 25, 0, 0, 0, 8, 6, 0, 0, 8, 4, 21, 0, 16, 0};
        vertex_t expected_dst_2_rank_7[]
            = {4, 4, 1, 1, 4, 0, 16, 1, 0, 16, 5, 25, 24, 8, 16, 2, 2, 0, 4, 16};
        ...
        size_t expected_len_0[]
            = {sizeof(expected_src_0_rank_0) / sizeof(expected_src_0_rank_0[0]),
        ...
        size_t expected_len_1[]
            = {sizeof(expected_src_1_rank_0) / sizeof(expected_src_1_rank_0[0]),
        ...
        size_t expected_len_2[]
            = {sizeof(expected_src_2_rank_0) / sizeof(expected_src_2_rank_0[0]),
        ...
        ret_code = hipgraph_generate_rmat_edgelists(handle,
                                                    rng_state,
                                                    num_lists,
                                                    4,
                                                    6,
        ...
        if(rank < max_rank_to_validate)
        {
            for(size_t i = 0; (i < num_lists) && (test_ret_value == 0); i++)
            {
                ...
                EXPECT_EQ(src_size, expected_len[i][rank]) << "wrong number of edges";
                ...
                {
                    EXPECT_EQ(expected_src[i][rank][j], h_src[j]) << "generated edges don't match";
                    EXPECT_EQ(expected_dst[i][rank][j], h_dst[j]) << "generated edges don't match";
                }
