...
namespace
{
    ...
    int generic_scc_test(const hipgraph_resource_handle_t* p_handle,
                         vertex_t*                         h_src,
                         vertex_t*                         h_dst,
                         weight_t*                         h_wgt,
                         vertex_t*                         h_result,
                         ...
                         hipgraph_bool_t                   store_transposed)
    {
        ...
        EXPECT_EQ(ret_code, HIPGRAPH_NOT_IMPLEMENTED) << "SCC should not be implemented, but is";

#if 0
  // FIXME: Actual implementation will be something like this
  EXPECT_EQ(ret_code, HIPGRAPH_SUCCESS) << "hipgraph_strongly_connected_components failed.";
  ...
  ret_code = hipgraph_type_erased_device_array_view_copy_to_host(
    handle, (hipgraph_byte_t*)h_vertices, vertices, &ret_error);
  EXPECT_EQ(ret_code, HIPGRAPH_SUCCESS) << "copy_to_host failed.";

  ret_code = hipgraph_type_erased_device_array_view_copy_to_host(
    handle, (hipgraph_byte_t*)h_components, components, &ret_error);
  EXPECT_EQ(ret_code, HIPGRAPH_SUCCESS) << "copy_to_host failed.";
  ...
  EXPECT_EQ(num_errors, 0) << "strongly connected components results don't match";
  ...
  hipgraph_labeling_result_free(p_result);
    ...
    int test_strongly_connected_components(const hipgraph_resource_handle_t* p_handle)
    {
        size_t num_edges    = 16;
