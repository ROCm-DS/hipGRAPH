...
namespace
{
    ...
    int generic_leiden_test(const hipgraph_resource_handle_t* p_handle,
                            vertex_t*                         h_src,
                            vertex_t*                         h_dst,
                            weight_t*                         h_wgt,
                            vertex_t*                         h_result,
                            ...
                            hipgraph_bool_t                   store_transposed)
    {
        ...
        ret_code = hipgraph_leiden(p_handle,
                                   rng_state,
                                   p_graph,
                                   max_level,
                                   resolution,
        ...
        EXPECT_EQ(ret_code, HIPGRAPH_SUCCESS)
            << "hipgraph_leiden failed. " << hipgraph_error_message(ret_error);
...
#if 0
    for (vertex_t i = 0; (i < num_local_vertices) && (test_ret_value == 0); ++i) {
      EXPECT_EQ(h_result[h_vertices[i]], component_mapping[h_clusters[i]]) << "cluster results don't match";
    }
    ...
    int test_leiden(const hipgraph_resource_handle_t* p_handle)
    {
        size_t   num_edges    = 8;
        ...
        return generic_leiden_test(p_handle,
                                   h_src,
                                   h_dst,
                                   h_wgt,
                                   h_result,
