...
def personalized_pagerank(ResourceHandle resource_handle,
    ...
    error_code = hipgraph_personalized_pagerank_allow_nonconvergence(
        ...
        &error_ptr)
    assert_success(
        error_code, error_ptr, "hipgraph_personalized_pagerank_allow_nonconvergence")
    ...
    if personalization_vertices is not None:
        hipgraph_type_erased_device_array_view_free(personalization_vertices_view_ptr)
    if personalization_values is not None:
        hipgraph_type_erased_device_array_view_free(personalization_values_view_ptr)
