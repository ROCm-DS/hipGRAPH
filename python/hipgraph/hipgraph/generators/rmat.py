...
from pylibhipgraph import generate_rmat_edgelist as pylibhipgraph_generate_rmat_edgelist
from pylibhipgraph import (
    generate_rmat_edgelists as pylibhipgraph_generate_rmat_edgelists,
)

_graph_types = [hipgraph.Graph, hipgraph.MultiGraph]
...
def _sg_rmat(
    ...
    min_edge_type_value,
    max_edge_type_value,
    create_using=hipgraph.Graph,
):
    """
    Calls RMAT on a single GPU and uses the resulting cuDF DataFrame
    to initialize and return a hipgraph Graph object specified with
    ...
    random_state = seed
    multi_gpu = False
    src, dst, weights, edge_id, edge_type = pylibhipgraph_generate_rmat_edgelist(
        ResourceHandle(),
        random_state,
        scale,
        num_edges,
...
def _mg_rmat(
    ...
    initialize and return a hipgraph Graph object specified with create_using.
...
def _call_rmat(
    ...
    return pylibhipgraph_generate_rmat_edgelist(
...
def rmat(
    ...
    create_using : hipgraph Graph type or None The graph type to construct
        ...
        and duplicated edges.  Default is hipgraph.Graph.

    UNIMPLEMENTED: mg : bool, optional (default=False)
        If True, R-MAT generation occurs across multiple GPUs. If False, only a
        single GPU is used.  Default is False (single-GPU).

    Returns
    -------
    instance of hipgraph.Graph or cudf or dask_cudf DataFrame
    ...
    if mg:
        raise "MG currently unsupported"
        return _mg_rmat(
            scale,
            num_edges,
            a,
...
def multi_rmat(
    ...
    Returns
    -------
    list of hipgraph.Graph instances
    """
    _ensure_args_rmat(
    ...
    edgelists = pylibhipgraph_generate_rmat_edgelists(
        ResourceHandle(),
        seed,
        n_edgelists,
        min_scale,
