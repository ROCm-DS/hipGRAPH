...
from hipgraph.gnn.comms import hipgraph_comms_get_raft_handle
from hipgraph.gnn.data_loading.dist_io import BufferedSampleReader, DistSampleWriter
...
class DistSampler:
    def __init__(
        self,
        graph: Union[pylibhipgraph.SGGraph, pylibhipgraph.MGGraph],
        ...
        graph: SGGraph or MGGraph (required)
            The pylibhipgraph graph object that will be sampled.
    ...
    def is_multi_gpu(self):
        return isinstance(self.__graph, pylibhipgraph.MGGraph)
    ...
    def _resource_handle(self):
        if self.__handle is None:
            if self.is_multi_gpu:
                self.__handle = pylibhipgraph.ResourceHandle(
                    hipgraph_comms_get_raft_handle().getHandle()
                )
            else:
                self.__handle = pylibhipgraph.ResourceHandle()
...
class NeighborSampler(DistSampler):
    ...
    def __init__(
        ...
        *,
        ...
        # if libhipgraph allows setting a new attribute, this API might
        # change.
        # TODO allow func to be a call to a future remote sampling API
        # if the provided graph is in another process (rapidsai/hipgraph#4623).
        self.__func = (
            pylibhipgraph.biased_neighbor_sample
            if biased
            else pylibhipgraph.uniform_neighbor_sample
        )
    ...
    def sample_batches(
        ...
        if self.is_multi_gpu:
            ...
            sampling_results_dict = self.__func(
                ...
                label_offsets=(
                    None if label_offsets is None else cupy.asarray(label_offsets)
                ),
                return_dict=True,
            )
