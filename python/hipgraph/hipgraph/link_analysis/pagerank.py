...
import cudf
import numpy as np
from hipgraph.exceptions import FailedToConvergeError
from hipgraph.utilities import df_score_to_dictionary, ensure_hipgraph_obj_for_nx
from pylibhipgraph import ResourceHandle
from pylibhipgraph import exceptions as plc_exceptions
from pylibhipgraph import pagerank as plc_pagerank
from pylibhipgraph import personalized_pagerank as plc_p_pagerank
...
def pagerank(
    ...
    """Find the PageRank score for every vertex in a graph. hipGRAPH computes an
    ...
    PageRank : cudf.DataFrame
        ...
        NOTE: if the input hipgraph.Graph was created using the renumber=False
        ...
        behavior, ensure the input hipgraph.Graph is created from the
    ...
    >>> pr = hipgraph.pagerank(G, alpha = 0.85, max_iter = 500, tol = 1.0e-05)
    """
    ...
    G, isNx = ensure_hipgraph_obj_for_nx(G, weight, store_transposed=True)
    ...
    # Re-raise this as a hipgraph exception so users trying to catch this do not
