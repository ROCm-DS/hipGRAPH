...
cdef class DeviceResources:
    ...
    stream : Optional stream to use for ordering CUDA instructions
             Accepts hipgraph._pylibraft.common.Stream() or uintptr_t (cudaStream_t)
    ...
    >>> from hipgraph._pylibraft.common import Stream, DeviceResources
    >>> stream = Stream()
    ...
    >>> import cupy
    >>> from hipgraph._pylibraft.common import Stream
    >>>
    >>> raft_stream = Stream()
    ...
    def __cinit__(self, stream=None, n_streams=0):
        ...
        # We should either have a hipgraph._pylibraft.common.Stream or a uintptr_t
        # of a cudaStream_t
        if stream is None:
            ...
            if isinstance(stream, Stream):
                # Stream is hipgraph._pylibraft Stream()
                s = stream.get_ptr()
...
cdef class Handle(DeviceResources):
    ...
    >>> from hipgraph._pylibraft.common import Stream, Handle
    >>> stream = Stream()
