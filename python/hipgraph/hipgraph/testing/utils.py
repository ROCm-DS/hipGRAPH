...
from hipgraph.dask.common.mg_utils import get_client
...
HIPGRAPH_INPUT_TYPES = [
    pytest.param(
        hipgraph.Graph(), marks=pytest.mark.hipgraph_types, id="hipgraph.Graph"
    ),
]

HIPGRAPH_DIR_INPUT_TYPES = [
    pytest.param(
        hipgraph.Graph(directed=True),
        marks=pytest.mark.hipgraph_types,
        id="hipgraph.Graph(directed=True)",
    ),
]
...
def create_obj_from_csv(
    ...
    csv_file_name
    """
    if obj_type in [hipgraph.Graph]:
        return generate_hipgraph_graph_from_file(
            csv_file_name,
            directed=directed,
            edgevals=edgevals,
        )
    elif isinstance(obj_type, hipgraph.Graph):
        return generate_hipgraph_graph_from_file(
            csv_file_name,
            directed=directed,
            edgevals=edgevals,
...
def generate_hipgraph_graph_from_file(graph_file, directed=True, edgevals=False):
...
def generate_mg_batch_hipgraph_graph_from_file(graph_file, directed=True):
    client = get_client()
...
def build_cu_and_nx_graphs(graph_file, directed=True, edgevals=False):
    G = generate_hipgraph_graph_from_file(
...
def build_mg_batch_cu_and_nx_graphs(graph_file, directed=True):
    G = generate_mg_batch_hipgraph_graph_from_file(graph_file, directed=directed)
...
def compare_mst(mst_hipgraph, mst_nx):
    mst_nx_df = nx.to_pandas_edgelist(mst_nx)
    edgelist_df = mst_hipgraph.view_edge_list()
    ...
    # check total weight
    cg_sum = edgelist_df[mst_hipgraph.weight_column].sum()
