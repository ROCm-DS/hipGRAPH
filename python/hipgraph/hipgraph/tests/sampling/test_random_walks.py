...
from hipgraph.testing import DEFAULT_DATASETS, SMALL_DATASETS
...
def calc_random_walks(G, max_depth=None, use_padding=False, legacy_result_type=True):
    ...
    G, _ = ensure_hipgraph_obj_for_nx(G, nx_weight_attr="wgt")
    ...
    print("\nstart_vertices is \n", start_vertices)
    vertex_paths, edge_weights, vertex_path_sizes = hipgraph.random_walks(
...
def test_random_walks_invalid_max_dept(graph_file, directed, max_depth):
    ...
    input_graph = graph_file.get_graph(create_using=hipgraph.Graph(directed=directed))
...
def test_random_walks_coalesced(graph_file, directed):
    ...
    df = hipgraph.rw_path(len(seeds), path_data[2])
...
def test_random_walks_padded_1():
    ...
    input_graph = hipgraph.Graph(directed=True)
...
def test_random_walks(
    ...
    df, offsets = hipgraph.random_walks(G, seeds, max_depth)
