...
import pandas
import pytest
from hipgraph.dask import uniform_neighbor_sample
from hipgraph.dask.common.mg_utils import is_single_gpu
from hipgraph.datasets import email_Eu_core, small_tree
from hipgraph.structure.symmetrize import (
    _memory_efficient_drop_duplicates,
    symmetrize_ddf,
)
from hipgraph.testing import UNDIRECTED_DATASETS
...
def input_combo(request):
    ...
    # sample k vertices from the hipGRAPH graph
...
def test_mg_uniform_neighbor_sample_ensure_no_duplicates(dask_client):
    ...
    mg_G = hipgraph.MultiGraph(directed=True)
    mg_G.from_dask_cudf_edgelist(
        dask_df, source="src", destination="dst", renumber=True
    )

    output_df = hipgraph.dask.uniform_neighbor_sample(
        mg_G,
...
def test_uniform_neighbor_sample_edge_properties(dask_client, return_offsets):
    ...
    G = hipgraph.MultiGraph(directed=True)
    ...
    sampling_results = hipgraph.dask.uniform_neighbor_sample(
        G,
...
def test_uniform_neighbor_edge_properties_sample_small_start_list(
    ...
    hipgraph.dask.uniform_neighbor_sample(
        G,
        start_list=dask_cudf.from_cudf(
            cudf.Series(
                {
...
def test_uniform_neighbor_sample_renumber(dask_client, hops):
    ...
    # a random seed. Right now, there are tests in hipGRAPH-DGL and
    # hipGRAPH-PyG that provide better coverage, but a better test
    ...
    sampling_results_renumbered, renumber_map = hipgraph.dask.uniform_neighbor_sample(
        G,
        seeds,
        hops,
        with_replacement=False,
...
def test_uniform_neighbor_sample_csr_csc_global(dask_client, hops, seed):
    ...
    sampling_results, offsets, renumber_map = hipgraph.dask.uniform_neighbor_sample(
        G,
        seeds,
        hops,
        with_replacement=False,
...
def bench_uniform_neighbor_sample_email_eu_core(gpubenchmark, dask_client, n_samples):
    ...
    def func():
        _ = hipgraph.dask.uniform_neighbor_sample(dg, start_list, [10])
        del _
