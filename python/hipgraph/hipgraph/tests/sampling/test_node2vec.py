...
import hipgraph
import pytest
from hipgraph.datasets import karate, small_line
...
def calc_node2vec(G, start_vertices, max_depth, compress_result, p=1.0, q=1.0):
    ...
    vertex_paths, edge_weights, vertex_path_sizes = hipgraph.node2vec(
...
def test_node2vec_line(graph_file, directed):
    G = graph_file.get_graph(
        download=True, create_using=hipgraph.Graph(directed=directed)
    )
    max_depth = 3
