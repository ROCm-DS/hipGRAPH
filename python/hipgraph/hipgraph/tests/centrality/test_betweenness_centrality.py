...
import numpy as np
import pytest
from hipgraph.datasets import karate_disjoint
from hipgraph.testing import SMALL_DATASETS, utils
...
def calc_betweenness_centrality(
    ...
    """Generate both hipgraph and networkx betweenness centrality
    Parameters
    ----------
    graph_file : string
...
def _calc_bc_subset(G, Gnx, normalized, weight, endpoints, k, seed, result_dtype):
    ...
    print("\nsources are ", sources)
    df = hipgraph.betweenness_centrality(
        G,
        k=sources,
        normalized=normalized,
        weight=weight,
...
def _calc_bc_subset_fixed(G, Gnx, normalized, weight, endpoints, k, seed, result_dtype):
    ...
    random.seed(seed)  # It will be called again in hipgraph's call
    ...
    # behavior
    df2 = hipgraph.betweenness_centrality(
        G,
        k=sources,
        normalized=normalized,
        weight=weight,
...
def test_betweenness_centrality_nx(graph_file, directed, edgevals):
    ...
    cu_bc = hipgraph.betweenness_centrality(Gnx)

    # Calculating mismatch
    networkx_bc = sorted(nx_bc.items(), key=lambda x: x[0])
    hipgraph_bc = sorted(cu_bc.items(), key=lambda x: x[0])
    err = 0
    assert len(hipgraph_bc) == len(networkx_bc)
    for i in range(len(hipgraph_bc)):
        if (
            abs(hipgraph_bc[i][1] - networkx_bc[i][1]) > 0.01
            and hipgraph_bc[i][0] == networkx_bc[i][0]
        ):
            err = err + 1
            print(f"{hipgraph_bc[i][1]} and {networkx_bc[i][1]}")
            print(f"{hipgraph_bc[i][0]} and {networkx_bc[i][0]}")
    print("Mismatches:", err)
    assert err < (0.01 * len(hipgraph_bc))
