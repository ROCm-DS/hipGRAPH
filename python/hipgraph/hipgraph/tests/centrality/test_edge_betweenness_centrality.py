...
def calc_edge_betweenness_centrality(
    ...
    """Generate both hipgraph and networkx edge betweenness centrality
...
def _calc_bc_subset(G, Gnx, normalized, weight, k, seed, result_dtype):
    ...
    df = hipgraph.edge_betweenness_centrality(
        G,
        k=sources,
        normalized=normalized,
        weight=weight,
...
def _calc_bc_subset_fixed(G, Gnx, normalized, weight, k, seed, result_dtype):
    ...
    # behavior
    df2 = (
        hipgraph.edge_betweenness_centrality(
            G,
            k=sources,
            normalized=normalized,
            weight=weight,
...
def test_edge_betweenness_centrality_nx(graph_file, directed, edgevals, normalized):
    ...
    cu_bc = hipgraph.edge_betweenness_centrality(Gnx, normalized=normalized)
    ...
    assert len(networkx_bc) == len(hipgraph_bc)
