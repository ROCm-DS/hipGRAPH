...
from hipgraph.structure.symmetrize import symmetrize_df
...
def get_sg_results(dataset, core_number, degree_type):
    ...
    if core_number:
        ...
        core_number = hipgraph.core_number(G, degree_type=degree_type)
    else:
        core_number = None

    sg_k_core_graph = hipgraph.k_core(
...
def test_sg_k_core(dask_client, dataset, core_number, degree_type, benchmark):
    ...
    if core_number:
        ...
        core_number = None
    sg_k_core = benchmark(
        hipgraph.k_core, G, core_number=core_number, degree_type=degree_type
    )
...
def test_dask_mg_k_core_invalid_input(dask_client):
    ...
    dg = dataset.get_dask_graph(create_using=hipgraph.Graph(directed=True))
