...
from hipgraph.datasets import cyber, netscience
...
# order to prevent hipGRAPH code from running on import, which would prevent
...
def setup_function():
    ...
    DiGraph_inst = hipgraph.Graph(directed=True)
...
def dataset1_MGPropertyGraph(dask_client):
    ...
    (merchants, users, transactions, relationships, referrals) = dataset1.values()
    from hipgraph.experimental import MGPropertyGraph
