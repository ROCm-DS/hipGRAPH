...
# Helper functions
# =============================================================================
def convert_output_to_cudf(input_G_or_matrix, hipgraph_result):
    """
    Convert hipgraph_result to a cudf DataFrame. The conversion is based on the
    type of input_G_or_matrix, since different input types result in different
    hipgraph_result types (see hipgraph_input_output_map).
    """
    input_type = type(input_G_or_matrix)
    expected_return_type = hipGRAPH_input_output_map[type(input_G_or_matrix)]
    assert type(hipgraph_result) is expected_return_type

    if expected_return_type is cudf.DataFrame:
        return hipgraph_result
    ...
    #      leading to the vertex at position i in the (input) vertex array.
        ...
        if input_type in cupy_types:
            assert type(hipgraph_result[0]) is cp.ndarray
            assert type(hipgraph_result[1]) is cp.ndarray
            if len(hipgraph_result) == 3:
                assert type(hipgraph_result[2]) is cp.ndarray
        else:
            assert type(hipgraph_result[0]) is np.ndarray
            assert type(hipgraph_result[1]) is np.ndarray
            if len(hipgraph_result) == 3:
                assert type(hipgraph_result[2]) is np.ndarray
        ...
        dists = [n.item() for n in hipgraph_result[0]]
        preds = [n.item() for n in hipgraph_result[1]]
        assert len(verts) == len(dists) == len(preds)

        d = {"vertex": verts, "distance": dists, "predecessor": preds}

        if len(hipgraph_result) == 3:
            counters = [n.item() for n in hipgraph_result[2]]
...
def compare_bfs(benchmark_callable, G, golden_values, start_vertex, depth_limit):
    """
    Generate both hipgraph and reference bfs traversal.
    """

    if isinstance(start_vertex, int):
        result = benchmark_callable(hipgraph.bfs_edges, G, start_vertex)
        hipgraph_df = convert_output_to_cudf(G, result)
        ...
        #       not contain all the vertices while the hipgraph version return
        #       a cudf.DataFrame with all the vertices, also some verification
        #       become slow with the data transfer
        compare_func(hipgraph_df, golden_values, start_vertex)

    elif isinstance(start_vertex, list):  # For other Verifications
        all_golden_values = golden_values
        all_hipgraph_distances = []

        def func_to_benchmark():
            for sv in start_vertex:
                hipgraph_df = hipgraph.bfs_edges(G, sv, depth_limit=depth_limit)
                all_hipgraph_distances.append(hipgraph_df)

        benchmark_callable(func_to_benchmark)

        compare_func = _compare_bfs
        for i, sv in enumerate(start_vertex):
            hipgraph_df = convert_output_to_cudf(G, all_hipgraph_distances[i])

            compare_func(hipgraph_df, all_golden_values[i], sv)

    else:  # Unknown type given to seed
        raise NotImplementedError("Invalid type for start_vertex")


def _compare_bfs(hipgraph_df, golden_distances, source):
    ...
    # pass a nullptr
    assert len(hipgraph_df.columns) == 3, (
        "The result of the BFS has an invalid " "number of columns"
    )
    cu_distances = {
        vertex: dist
        for vertex, dist in zip(
            hipgraph_df["vertex"].to_numpy(), hipgraph_df["distance"].to_numpy()
        )
    }
    cu_predecessors = {
        vertex: dist
        for vertex, dist in zip(
            hipgraph_df["vertex"].to_numpy(), hipgraph_df["predecessor"].to_numpy()
        )
    }
                ...
                if vertex != source and pred not in golden_distances:
                    ...
                    if vertex != source and (
                        ...
                        print(
                            ...
                            "vid = {}, hipgraph = {}".format(vertex, pred)
                        )
                        invalid_predecessor_error += 1
        else:
            missing_vertex_error += 1
...
def get_cu_graph_and_params(dataset, directed):
    """
    Helper for fixtures returning a hipGRAPH obj and params.
    """
    # create graph
...
# =============================================================================
@pytest.mark.sg
@pytest.mark.parametrize("hipgraph_input_type", utils.HIPGRAPH_INPUT_TYPES)
def test_bfs(gpubenchmark, dataset_goldenresults_startvertex_spc, hipgraph_input_type):
    """
    ...
    if directed:
        if isinstance(hipgraph_input_type, hipgraph.Graph):
            hipgraph_input_type = hipgraph.Graph(directed=True)

    if not isinstance(hipgraph_input_type, hipgraph.Graph):
        G_or_matrix = utils.create_obj_from_csv(dataset, hipgraph_input_type)
    else:
        G_or_matrix = G
...
def test_bfs_nonnative_inputs_matrix(
    gpubenchmark, single_dataset_goldenresults_startvertex_spc, hipgraph_input_type
):
    test_bfs(
        gpubenchmark, single_dataset_goldenresults_startvertex_spc, hipgraph_input_type
    )
...
@pytest.mark.sg
@pytest.mark.parametrize("hipgraph_input_type", utils.HIPGRAPH_INPUT_TYPES)
def test_bfs_invalid_start(dataset_goldenresults_startvertex_spc, hipgraph_input_type):
    ...
    with pytest.raises(ValueError):
        hipgraph.bfs(G, start_vertex, depth_limit=depth_limit)
...
def test_scipy_api_compat():
    ...
    input_hipgraph_graph = graph_file.get_graph(ignore_weights=True)
    ...
    with pytest.raises(TypeError):
        hipgraph.bfs(input_hipgraph_graph, start=0, directed=False)
    with pytest.raises(TypeError):
        hipgraph.bfs(input_hipgraph_graph)  # required arg missing

    # Ensure hipgraph-compatible options work as expected
    hipgraph.bfs(input_hipgraph_graph, i_start=0)
    hipgraph.bfs(input_hipgraph_graph, i_start=0)
    # cannot have start and i_start
    with pytest.raises(TypeError):
        hipgraph.bfs(input_hipgraph_graph, start=0, i_start=0)

    # Ensure SciPy options for matrix inputs work as expected
    hipgraph.bfs(input_coo_matrix, i_start=0)
    hipgraph.bfs(input_coo_matrix, i_start=0, directed=True)
    hipgraph.bfs(input_coo_matrix, i_start=0, directed=False)
    result = hipgraph.bfs(input_coo_matrix, i_start=0)
