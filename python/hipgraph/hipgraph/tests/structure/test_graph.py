...
from hipgraph.dask.traversal.bfs import convert_to_cudf
from hipgraph.testing import utils
from pylibhipgraph import ResourceHandle
from pylibhipgraph import bfs as pylibhipgraph_bfs
...
def compare_graphs(nx_graph, cu_graph):
    ...
    if len(edgelist_df.columns) > 2:
        ...
        merge = df.merge(df0, on=["source", "target"], suffixes=("_hipgraph", "_nx"))
        print("merge = \n", merge)
        print(merge[merge.weight_hipgraph != merge.weight_nx])
        if not merge["weight_hipgraph"].equals(merge["weight_nx"]):
...
def test_add_edge_list_to_adj_list(graph_file):
    ...
    # hipgraph add_egde_list to_adj_list call
...
def test_add_adj_list_to_edge_list(graph_file):
    ...
    # hipgraph add_adj_list to_edge_list call
    G = hipgraph.Graph(directed=True)
...
def test_delete_edge_list_delete_adj_list(graph_file):
    ...
    # hipgraph delete_adj_list delete_edge_list call
    G = hipgraph.Graph(directed=True)
...
def test_add_edge_or_adj_list_after_add_edge_or_adj_list(graph_file):
    ...
    # If hipgraph has at least one graph representation, adding a new graph
    # should fail to prevent a single graph object storing two different
    # graphs.

    # If hipgraph has a graph edge list, adding a new graph should fail.
    G.from_cudf_edgelist(df, source="src", destination="dst")
    with pytest.raises(Exception):
        G.from_cudf_edgelist(df, source="src", destination="dst")
    with pytest.raises(Exception):
    ...
    # If hipgraph has a graph adjacency list, adding a new graph should fail.
    G.from_cudf_adjlist(offsets, indices, None)
    with pytest.raises(Exception):
        G.from_cudf_edgelist(df, source="src", destination="dst")
    with pytest.raises(Exception):
...
def test_edges_for_Graph(graph_file):
    ...
    # Create hipGRAPH Graph from DataFrame
    # Force it to use renumber_from_cudf
    G = hipgraph.from_cudf_edgelist(
        cu_M, source=["0"], destination=["1"], create_using=hipgraph.Graph
    )
    ...
    # Compare nx and hipgraph edges when viewing edgelist
    edges = []
...
def test_create_sg_graph(graph_file):
    ...
    G = hipgraph.from_cudf_edgelist(el, source=["0"], destination=["1"], edge_attr="2")
