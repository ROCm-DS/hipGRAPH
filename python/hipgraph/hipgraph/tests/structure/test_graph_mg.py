...
def test_create_mg_graph(dask_client, input_combo):
    ...
    res = [
        dask_client.submit(
            lambda sID, mg_graph_x, st_x: pylibhipgraph_bfs(
    ...
    g = hipgraph.Graph(directed=G.properties.directed)
    g.from_cudf_edgelist(df, "src", "dst")
    expected_dist = hipgraph.bfs(g, cudf.Series([1], dtype="int32"))
...
def test_graph_creation_properties(dask_client, graph_file, directed, renumber):
    ...
    sG = hipgraph.Graph(directed=directed)
    mG = hipgraph.Graph(directed=directed)
