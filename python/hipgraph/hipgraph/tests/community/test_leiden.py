...
def input_and_expected_output(request):
    ...
    parts, mod = hipgraph.leiden(G, max_level, resolution)
...
def hipgraph_leiden(G):

    # hipgraph Louvain Call
    t1 = time.time()
    parts, mod = hipgraph.leiden(G)
    t2 = time.time() - t1
    print("hipGRAPH Leiden Time : " + str(t2))

    return parts, mod


def hipgraph_louvain(G):

    # hipgraph Louvain Call
    t1 = time.time()
    parts, mod = hipgraph.louvain(G)
    t2 = time.time() - t1
    print("hipGRAPH Louvain Time : " + str(t2))
...
def test_leiden(graph_file):
    ...
    leiden_parts, leiden_mod = hipgraph_leiden(G)
    louvain_parts, louvain_mod = hipgraph_louvain(G)
