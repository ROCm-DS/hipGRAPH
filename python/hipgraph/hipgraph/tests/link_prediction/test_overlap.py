...
def hipgraph_call(benchmark_callable, graph_file, pairs, use_weight=False):
    # Device data
    G = graph_file.get_graph(
        create_using=hipgraph.Graph(directed=False), ignore_weights=not use_weight
    )
    # hipgraph Overlap Call
    df = benchmark_callable(hipgraph.overlap, G, pairs, use_weight=use_weight)
...
def test_overlap(gpubenchmark, read_csv, extract_two_hop, use_weight):
    ...
    cu_coeff = hipgraph_call(gpubenchmark, graph_file, pairs, use_weight=use_weight)
...
def test_overlap_multi_column(graph_file, use_weight):
    ...
    df_multi_col_res = hipgraph.overlap(G1, vertex_pair, use_weight=use_weight)
    G2 = hipgraph.Graph()
    G2.from_cudf_edgelist(
        cu_M, source=MULTI_COL_SRC_0_COL, destination=MULTI_COL_DST_0_COL, weight=weight
    )
    df_single_col_res = hipgraph.overlap(
...
def test_weighted_overlap():
    ...
    with pytest.raises(ValueError):
        hipgraph.overlap(G, use_weight=True)
...
def test_all_pairs_overlap():
    ...
    # Call Overlap
    overlap_results = hipgraph.overlap(G)
    ...
    all_pairs_overlap_results = hipgraph.all_pairs_overlap(G)
...
def test_all_pairs_overlap_with_vertices():
    ...
    all_pairs_overlap_results = hipgraph.all_pairs_overlap(
...
def test_all_pairs_overlap_with_topk():
    ...
    all_pairs_overlap_results = (
        hipgraph.all_pairs_overlap(G, topk=topk)
