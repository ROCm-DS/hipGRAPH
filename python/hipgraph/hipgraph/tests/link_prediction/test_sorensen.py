...
def compare_sorensen_two_hop(G, Gnx, use_weight=False):
    """
    Compute both hipgraph and nx sorensen after extracting the two hop neighbors
    ...
    df = hipgraph.sorensen(G, pairs, use_weight=use_weight)
    ...
    # hardcode it and compare to hipGRAPH Sorensen to get a more robust test
    #
    # AMD: The comment above cannot work in general. Results depend on the order
    # of operations.
...
def hipgraph_call(benchmark_callable, graph_file, input_df=None, use_weight=False):
    ...
    # If no vertex_pair is passed as input, 'hipgraph.sorensen' will
    ...
    # comparaison, get the one_hop_neighbor of the entire graph for 'hipgraph.sorensen'
    ...
    # hipgraph Sorensen Call
    df = benchmark_callable(hipgraph.sorensen, G, vertex_pair=vertex_pair)
...
def test_weighted_sorensen():
    ...
    with pytest.raises(ValueError):
        hipgraph.sorensen(G, use_weight=True)
