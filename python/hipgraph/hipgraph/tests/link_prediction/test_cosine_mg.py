...
def input_expected_output(input_combo):
    ...
    (based on hipGRAPH Cosine) which can be used for validation.
    """
    ...
    sg_hipgraph_cosine = hipgraph.cosine(
    ...
    input_combo["sg_hipgraph_results"] = sg_hipgraph_cosine
...
def input_expected_output_all_pairs(input_combo):
    ...
    sg_hipgraph_all_pairs_cosine = hipgraph.all_pairs_cosine(
        G,
    ...
    input_combo["sg_hipgraph_results"] = sg_hipgraph_all_pairs_cosine
    chunksize = dcg.get_chunksize(input_data_path)
    ddf = dask_cudf.read_csv(
        input_data_path,
        blocksize=chunksize,
...
def test_dask_mg_cosine(dask_client, benchmark, input_expected_output):
    ...
    result_cosine = (
        ...
        .rename(columns={"cosine_coeff": "mg_hipgraph_cosine_coeff"})
    )
    ...
    # Update the dask hipgraph Cosine results with sg hipgraph results for easy
    # comparison using cuDF DataFrame methods.
    result_cosine["sg_hipgraph_cosine_coeff"] = expected_output["cosine_coeff"]

    cosine_coeff_diffs1 = result_cosine.query(
        "mg_hipgraph_cosine_coeff - sg_hipgraph_cosine_coeff > 0.00001"
    )
