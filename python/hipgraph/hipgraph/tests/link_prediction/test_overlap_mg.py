...
def input_expected_output(input_combo):
    ...
    (based on hipGRAPH overlap) which can be used for validation.
    """
    ...
    sg_hipgraph_overlap = hipgraph.overlap(
    ...
    input_combo["sg_hipgraph_results"] = sg_hipgraph_overlap
...
def input_expected_output_all_pairs(input_combo):
    ...
    sg_hipgraph_all_pairs_overlap = hipgraph.all_pairs_overlap(
        G,
    ...
    input_combo["sg_hipgraph_results"] = sg_hipgraph_all_pairs_overlap
    chunksize = dcg.get_chunksize(input_data_path)
    ddf = dask_cudf.read_csv(
        input_data_path,
        blocksize=chunksize,
...
def test_dask_mg_overlap(dask_client, benchmark, input_expected_output):
    ...
    result_overlap = (
        ...
        .rename(columns={"overlap_coeff": "mg_hipgraph_overlap_coeff"})
    )
    ...
    # Update the dask hipgraph overlap results with sg hipgraph results for easy
    # comparison using cuDF DataFrame methods.
    result_overlap["sg_hipgraph_overlap_coeff"] = expected_output["overlap_coeff"]

    overlap_coeff_diffs1 = result_overlap.query(
        "mg_hipgraph_overlap_coeff - sg_hipgraph_overlap_coeff > 0.00001"
    )
