...
def compare_jaccard_two_hop(G, Gnx, use_weight=False):
    """
    Compute both hipgraph and nx jaccard after extracting the two hop neighbors
...
def hipgraph_call(benchmark_callable, graph_file, input_df=None, use_weight=False):
    G = hipgraph.Graph()
    G = graph_file.get_graph(ignore_weights=not use_weight)

    # If no vertex_pair is passed as input, 'hipgraph.jaccard' will
    ...
    # comparaison, get the one_hop_neighbor of the entire graph for 'hipgraph.jaccard'
    ...
    # hipgraph Jaccard Call
    df = benchmark_callable(
        hipgraph.jaccard, G, vertex_pair=vertex_pair, use_weight=use_weight
    )
...
def read_csv(request):
    """
    Read csv file for both networkx and hipgraph
    """
...
def test_jaccard(read_csv, gpubenchmark, use_weight):
    ...
    cu_src, cu_dst, cu_coeff = hipgraph_call(
...
def test_directed_graph_check(read_csv, use_weight):
    ...
    with pytest.raises(ValueError):
        hipgraph.jaccard(G1, vertex_pair, use_weight)
...
def test_jaccard_nx(read_csv):
    ...
    cg_j = hipgraph.jaccard_coefficient(Gnx, ebunch=ebunch)

    assert len(nv_js) > len(cg_j)

    # FIXME:  Nx does a full all-pair Jaccard.
    # hipGRAPH does a limited 1-hop Jaccard
    # assert nx_j == cg_j
...
def test_weighted_jaccard():
    ...
    with pytest.raises(ValueError):
        hipgraph.jaccard(G, use_weight=True)
...
def test_all_pairs_jaccard():
    ...
    # Call Jaccard
    jaccard_results = hipgraph.jaccard(G)
    ...
    all_pairs_jaccard_results = hipgraph.all_pairs_jaccard(G)
...
def test_all_pairs_jaccard_with_vertices():
    ...
    all_pairs_jaccard_results = hipgraph.all_pairs_jaccard(
...
def test_all_pairs_jaccard_with_topk():
    ...
    all_pairs_jaccard_results = (
        hipgraph.all_pairs_jaccard(G, topk=topk)
