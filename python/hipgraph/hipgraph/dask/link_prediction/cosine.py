...
def _call_plc_all_pairs_cosine(
    ...
    return pylibhipgraph_all_pairs_cosine_coefficients(
...
def _call_plc_cosine(
    ...
    return pylibhipgraph_cosine_coefficients(
        resource_handle=ResourceHandle(Comms.get_handle(sID).getHandle()),
        graph=mg_graph_x,
        first=first,
        second=second,
...
def cosine(input_graph, vertex_pair=None, use_weight=False):
    ...
    hipgraph.dask.cosine, in the absence of a specified vertex pair list, will
