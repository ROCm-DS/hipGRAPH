...
from pylibhipgraph import (
    all_pairs_sorensen_coefficients as pylibhipgraph_all_pairs_sorensen_coefficients,
)
from pylibhipgraph import sorensen_coefficients as pylibhipgraph_sorensen_coefficients
...
def sorensen(
    ...
    hipgraph.sorensen, in the absence of a specified vertex pair list, will
    ...
    do_expensive_check : bool, optional (default=False)
        ...
        values from 0 to V-1. That check is now redundant because hipgraph
    ...
    first, second, sorensen_coeff = pylibhipgraph_sorensen_coefficients(
...
def sorensen_coefficient(
    ...
    G : hipgraph.Graph or NetworkX.Graph
        hipGRAPH or NetworkX Graph instance, should contain the connectivity
    ...
    --------
    >>> from hipgraph.datasets import karate
    >>> from hipgraph import sorensen_coefficient
    ...
    # 'ebunch'(if not None) are respectively of type hipgraph.Graph and cudf.DataFrame?
...
def all_pairs_sorensen(
    ...
    hipgraph.all_pairs_sorensen, in the absence of specified vertices, will
    compute the two_hop_neighbors of the entire graph to construct a vertex pair
    ...
    --------
    >>> from hipgraph.datasets import karate
    >>> from hipgraph import all_pairs_sorensen
    >>> input_graph = karate.get_graph(download=True, ignore_weights=True)
    ...
    first, second, sorensen_coeff = pylibhipgraph_all_pairs_sorensen_coefficients(
        resource_handle=ResourceHandle(),
        graph=input_graph._plc_graph,
        vertices=vertices,
        use_weight=use_weight,
