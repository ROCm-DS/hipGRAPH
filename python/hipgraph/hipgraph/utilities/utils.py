...
def get_traversed_path(df, id):
    ...
    >>> sssp_df = hipgraph.sssp(G, 1)
    >>> path = hipgraph.utils.get_traversed_path(sssp_df, 32)
    >>> path
...
def get_traversed_path_list(df, id):
    ...
    >>> G.from_cudf_edgelist(gdf, source='0', destination='1', edge_attr='2')
    >>> sssp_df = hipgraph.sssp(G, 1)
    >>> path = hipgraph.utils.get_traversed_path_list(sssp_df, 32)
...
def ensure_hipgraph_obj(obj, nx_weight_attr=None, matrix_graph_type=None):
    """
    Convert the input obj - if possible - to a hipGRAPH Graph-type obj (Graph,
    etc.) and return a tuple of (hipgraph Graph-type obj, original
    input obj type). If matrix_graph_type is specified, it is used as the
    hipgraph Graph-type obj to create when converting from a matrix type.
    """
    # FIXME: importing here to avoid circular import
    from hipgraph.structure import Graph
    from hipgraph.utilities.nx_factory import convert_from_nx

    input_type = type(obj)
    if is_hipgraph_graph_type(input_type):
        ...
        if matrix_graph_type is None:
            ...
            if not isinstance(matrix_graph_type, Graph):
                raise TypeError(
                    f"matrix_graph_type must be either a hipgraph "
        ...
        # * populate the hipgraph graph with directed data and set renumbering
...
# |      Many NetworkX algorithms designed for weighted graphs use
# |      an edge attribute (by default `weight`) to hold a numerical value.
def ensure_hipgraph_obj_for_nx(
    obj, nx_weight_attr="weight", store_transposed=False, vertex_type="int32"
):
    """
    Ensures a hipGRAPH Graph-type obj is returned for either hipGRAPH or Nx
    ...
    if is_nx_graph_type(input_type):
        warn(
            ...
            "deprecated. Please use NetworkX with the nx-hipgraph backend",
            DeprecationWarning,
            2,
        )
        return (
    ...
    elif is_hipgraph_graph_type(input_type):
        return (obj, False)
    else:
        raise TypeError(
            "input must be either a hipGRAPH or NetworkX graph "
...
def import_optional(mod, default_mod_class=MissingModule):
    ...
    Example
    -------
    >> from hipgraph.utils import import_optional
