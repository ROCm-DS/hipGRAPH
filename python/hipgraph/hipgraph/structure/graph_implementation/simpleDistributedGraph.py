...
from hipgraph.dask.common.mg_utils import run_gc_on_dask_cluster
...
from hipgraph.structure.symmetrize import _memory_efficient_drop_duplicates
...
class simpleDistributedGraphImpl:
    ...
    def degrees_function(
        ...
        def _call_plc_degrees_function(
            ...
            if degree_type == "in_degree":
                results = pylibhipgraph_in_degrees(
                    resource_handle=ResourceHandle(Comms.get_handle(sID).getHandle()),
                    graph=mg_graph_x,
                ...
                results = pylibhipgraph_out_degrees(
                    resource_handle=ResourceHandle(Comms.get_handle(sID).getHandle()),
                    graph=mg_graph_x,
                    source_vertices=source_vertices,
                    do_expensive_check=False,
    ...
    def get_two_hop_neighbors(self, start_vertices=None):
        ...
        def _call_plc_two_hop_neighbors(sID, mg_graph_x, start_vertices):
            results_ = pylibhipgraph_get_two_hop_neighbors(
                resource_handle=ResourceHandle(Comms.get_handle(sID).getHandle()),
                graph=mg_graph_x,
    ...
    def decompress_to_edgelist(
        ...
        def _call_decompress_to_edgelist(
            ...
            return pylibhipgraph_decompress_to_edgelist(
                resource_handle=ResourceHandle(Comms.get_handle(sID).getHandle()),
                graph=mg_graph_x,
    ...
    def select_random_vertices(
        ...
        def _call_plc_select_random_vertices(
            ...
            cp_arrays = pylibhipgraph_select_random_vertices(
                graph=mg_graph_x,
                resource_handle=ResourceHandle(Comms.get_handle(sID).getHandle()),
    ...
    def to_undirected(self, G):
        ...
        >>> M = dask_cudf.read_csv(datasets_path / 'karate.csv', delimiter=' ',
        ...         dtype=['int32', 'int32', 'float32'], header=None)
        >>> DiG = hipgraph.Graph(directed=True)
