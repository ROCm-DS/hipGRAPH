...
# import dask_cudf
# import hipgraph.dask.common.mg_utils as mg_utils
# import hipgraph.dask.comms.comms as Comms
import numpy as np
import pandas as pd

# from hipgraph.dask.structure import replication
from hipgraph.structure import graph_primtypes_wrapper
from hipgraph.structure.number_map import NumberMap
from hipgraph.structure.replicate_edgelist import replicate_cudf_dataframe
from hipgraph.structure.symmetrize import symmetrize as symmetrize_df
from pylibhipgraph import GraphProperties, ResourceHandle, SGGraph
from pylibhipgraph import decompress_to_edgelist as pylibhipgraph_decompress_to_edgelist
from pylibhipgraph import degrees as pylibhipgraph_degrees
from pylibhipgraph import get_two_hop_neighbors as pylibhipgraph_get_two_hop_neighbors
from pylibhipgraph import in_degrees as pylibhipgraph_in_degrees
from pylibhipgraph import out_degrees as pylibhipgraph_out_degrees
from pylibhipgraph import select_random_vertices as pylibhipgraph_select_random_vertices
...
class simpleGraphImpl:
    ...
    def view_edge_list(self):
        ...
        could be different i.e. an edge displayed by hipgraph as (src, dst)
        could be displayed as (dst, src) by networkx.
        hipgraph.Graph stores symmetrized edgelist internally. For displaying
        ...
        hipgraph is different, the upper triangular matrix of networkx
        renumbered input may not be the same as hipgraph's upper trianglar
    ...
    def __from_adjlist(
        ...
        self._make_plc_graph(
            ...
            renumber=renumber,
            symmetrize=(
                not self.properties.directed if symmetrize is None else symmetrize
            ),
        )
    ...
    def get_two_hop_neighbors(self, start_vertices=None):
        ...
        first, second = pylibhipgraph_get_two_hop_neighbors(
    ...
    def decompress_to_edgelist(
        ...
        (
            ...
            edge_ids,
            edge_type_ids,
        ) = pylibhipgraph_decompress_to_edgelist(
            resource_handle=ResourceHandle(),
            graph=self._plc_graph,
            do_expensive_check=do_expensive_check,
        )
    ...
    def select_random_vertices(
        ...
        """
        vertices = pylibhipgraph_select_random_vertices(
            resource_handle=ResourceHandle(),
            graph=self._plc_graph,
    ...
    def degrees_function(
        ...
        if degree_type == "in_degree":
            vertex, in_degrees = pylibhipgraph_in_degrees(
                resource_handle=ResourceHandle(),
                graph=self._plc_graph,
            ...
            vertex, out_degrees = pylibhipgraph_out_degrees(
                resource_handle=ResourceHandle(),
                graph=self._plc_graph,
                source_vertices=vertex_subset,
                do_expensive_check=do_expensive_check,
            ...
            vertex, in_degrees, out_degrees = pylibhipgraph_degrees(
                resource_handle=ResourceHandle(),
                graph=self._plc_graph,
                source_vertices=vertex_subset,
                do_expensive_check=do_expensive_check,
