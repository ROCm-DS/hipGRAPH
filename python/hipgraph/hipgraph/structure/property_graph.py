...
class EXPERIMENTAL__PropertyGraph:
    ...
    def get_num_vertices(self, type=None, *, include_edge_data=True):
        ...
        >>> import hipgraph
        >>> import cudf
        >>> from hipgraph.experimental import PropertyGraph
    ...
    def get_vertex_data(self, vertex_ids=None, types=None, columns=None):
        ...
        if self.__vertex_prop_dataframe is not None:
            ...
            # Preserve the dtype (vertex id type) to avoid hipgraph algorithms
    ...
    def get_edge_data(self, edge_ids=None, types=None, columns=None):
        ...
        if self.__edge_prop_dataframe is not None:
            ...
            # Preserve the dtype (edge id type) to avoid hipgraph algorithms
            # throwing errors due to a dtype mismatch
    ...
    def extract_subgraph(
        ...
        Parameters
        ----------
        create_using : type or instance of hipgraph.Graph or PropertyGraph, optional
            ...
            returned Graph will be a directed hipgraph.MultiGraph instance.
        selection : PropertySelection, optional
            A PropertySelection returned from one or more calls to
        ...
        # prevent hipgraph from running on import. This may help diagnose errors
        create_kind = "hipgraph"
        if create_using is None:
            create_using = hipgraph.MultiGraph(directed=True)
    ...
    def annotate_dataframe(self, df, G, edge_vertex_col_names):
        ...
        G : hipgraph.Graph (or subclass of) instance.
        ...
        >>> pG = PropertyGraph()
        >>> pG.add_edge_data(df, type_name="etype", vertex_col_names=("src", "dst"))
        >>> G = pG.extract_subgraph(create_using=hipgraph.Graph(directed=True))
    ...
    def edge_props_to_graph(
        ...
        create_using : hipgraph.Graph (or subclass of) instance.
        ...
        elif type(create_using) is type and issubclass(create_using, hipgraph.Graph):
            G = create_using()
        else:
            raise TypeError(
