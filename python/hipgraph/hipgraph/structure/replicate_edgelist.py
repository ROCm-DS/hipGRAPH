...
# from pylibhipgraph import replicate_edgelist as pylibhipgraph_replicate_edgelist
...
def _call_plc_replicate_edgelist(
    ...
    cp_arrays = pylibhipgraph_replicate_edgelist(
...
def _call_plc_replicate_dataframe(sID: bytes, df: cudf.DataFrame) -> cudf.DataFrame:
    ...
    for col_name in df.columns:
        cp_array = pylibhipgraph_replicate_edgelist(
            resource_handle=ResourceHandle(Comms.get_handle(sID).getHandle()),
            src_array=(
...
def _mg_call_plc_replicate(
    ...
    if True:
        raise NotImplementedError("Multi-GPU support currently is not implemented.")
