...
## Description
[RAPIDS](https://rapids.ai) nx-hipgraph is a [backend to NetworkX](https://networkx.org/documentation/stable/reference/utils.html#backends)
to run supported algorithms with GPU acceleration.

## System Requirements

nx-hipgraph requires the following:
...
nx-hipgraph can be installed using either conda or pip.

### conda
#### latest nightly version
```
conda install -c rapidsai-nightly -c conda-forge -c nvidia nx-hipgraph
```
#### latest stable version
```
conda install -c rapidsai -c conda-forge -c nvidia nx-hipgraph
```
### pip
#### latest nightly version
```
python -m pip install nx-hipgraph-cu11 --extra-index-url https://pypi.anaconda.org/rapidsai-wheels-nightly/simple
```
#### latest stable version
```
python -m pip install nx-hipgraph-cu11 --extra-index-url https://pypi.nvidia.com
```
Notes:
...
## Enabling nx-hipgraph

NetworkX will use nx-hipgraph as the graph analytics backend if any of the
following are used:
...
Set `NETWORKX_AUTOMATIC_BACKENDS=hipgraph` to use nx-hipgraph to GPU accelerate supported APIs with no code changes.
Example:
```
bash> NETWORKX_AUTOMATIC_BACKENDS=hipgraph python my_networkx_script.py
```
...
Example:
```
nx.betweenness_centrality(cit_patents_graph, k=k, backend="hipgraph")
```
...
To use type-based dispatching with nx-hipgraph, the user must import the backend
directly in their code to access the utilities provided to create a Graph
instance specifically for the nx-hipgraph backend.

Example:
```
import networkx as nx
import nx_hipgraph as nxhg

G = nx.Graph()
...
nxhg_G = nxhg.from_networkx(G)             # conversion happens once here
nx.betweenness_centrality(nxhg_G, k=1000)  # nxcg Graph type causes hipgraph backend
                                           # to be used, no conversion necessary
```

## Supported Algorithms

The nx-hipgraph backend to NetworkX connects
[pylibhipgraph](../../readme_pages/pylibhipgraph.md) (hipgraph's low-level python
...
Below is the list of algorithms that are currently supported in nx-hipgraph.
...
To request nx-hipgraph backend support for a NetworkX API that is not listed
above, visit the [hipgraph GitHub repo](https://github.com/rapidsai/hipgraph).
