...
if TYPE_CHECKING:  # pragma: no cover
    ...
    from nx_hipgraph.typing import (
        AttrKey,
        Dtype,
        EdgeTuple,
        EdgeValue,
...
class Graph:
    # Tell networkx to dispatch calls with this object to nx-hipgraph
    __networkx_backend__: ClassVar[str] = "hipgraph"  # nx >=3.2
    __networkx_plugin__: ClassVar[str] = "hipgraph"  # nx <3.2
                ...
                if edge_array.dtype == np.int64:
                    if (val := edge_array.max().tolist()) > 2**53:
                        raise ValueError(
                            ...
                            "pylibhipgraph only supports float16 and float32 dtypes."
                        )
